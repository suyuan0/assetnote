...
Known baseline gaps scheduled for the remaining hardening steps are the Web TypeScript path mapping that resolves `@workspace/ui/*` directly to package source, the UI stylesheet's invalid outward Tailwind source path, the absence of a non-mutating root verification command, API e2e tests not being part of the root task graph, and the absence of CI. Tailwind content scanning must remain an application responsibility; fixing the path must not create a package-to-application dependency.
...
| Framework-local conventions | Workspace `AGENTS.md`                     | Partially implemented: `pnpm lint:check`; typecheck, tests, and build remain planned |
